...
class QueryBuilder {
    ...
    static async BRANCH_LIST (req, skipPageLimit){
      ...
      query.where = {}
      query.include = [{model: sequelize.modelManager.getModel('branchLocation')}]
      ...
      if(reqData.search){
        ...
        if(_.isUndefined(reqData.byLocation)){
          ...
          query.include = [{model: sequelize.modelManager.getModel('branchLocation'), where: {
            [Op.or]: [
              {
                id: {
                  [Op.like]: `%${reqData.search}%`
      ...
      if(!_.isUndefined(reqData.visit) && reqData.visit.toLowerCase() === "true"){
        callFunction = "findAndCountAll"
        if(!_.isUndefined(reqData.visitStatus)){
          query.include.push({model: sequelize.modelManager.getModel('visit'), where: {
      ...
      if(!_.isUndefined(reqData.appointment) && reqData.appointment.toLowerCase() === "true"){
        callFunction = "findAndCountAll"
        if(!_.isUndefined(reqData.appointmentStatus)){
          query.include.push({model: sequelize.modelManager.getModel('appointment'), where: {
            status: reqData.appointmentStatus.toUpperCase()
          }})
        }else{
          query.include.push({model: sequelize.modelManager.getModel('appointment')})
        }
      }
    ...
    static async LIST_VISIT (req) {
      const reqData = req.query;

      let db_query = "SELECT * FROM visit;";
    ...
    static serviceList (req, query, skipPageLimit = false) {
        ...
        let selectFields = 'code name description status createdBy createdAt updatedAt'
  ...
  static healthFacilitiesList (req, query, skipPageLimit = false) {
    const reqData = req.query
    let sortField = '-createdAt'
    let selectFields = 'name code locationCode province district sector cell village status category type updatedAt'
  ...
  static nursesList (req, query, skipPageLimit = false) {
    const reqData = req.query
    let sortField = '-createdAt'
    let selectFields = 'idType idNumber facilityId surName postNames dateOfBirth maritalStatus nationality domicileCountry domicileDistrict domicileProvince domicileSector domicileCell domicileVillage licenseNumber qualification updatedAt'
    ...
    query.populate({
      path: 'facilityId', select: 'name code locationCode province district sector cell village status'
    })
  ...
  static embassiesList (req, skipPageLimit = false) {
    const reqData = req.query
    const limit = Number(reqData.limit ? reqData.limit : process.env.PAGE_LIMIT)
    ...
    if (reqData.associatedCountries) {
      embassyMatch.$match.associatedCountries = { $in: [reqData.associatedCountries] }
    }
    ...
    if (reqData.facilityId) {
      embassyMatch.$match._id = { $eq: new mongoose.Types.ObjectId(reqData.facilityId) }
    }
    ...
    if (!skipPageLimit) {
      resultArray = [{ $sort: { [`${sortField}`]: order } }, { $skip: pageNo }, { $limit: limit }]
    }
  ...
  static nursesAndAttendantsList (req, type, skipPageLimit = false) {
    ...
    if (reqData.documentNumber) {
      nurseMatch.$match.documentNumber = new RegExp(reqData.documentNumber, 'i')
    }
      ...
      match.$match.$and.push({
        $or: [
          { status: STATUS.ACTIVE },
          {
            ...
            dateOfDeath: { $gt: new Date(Date.now() - 1000 * 3600 * 24 * NURSE_ATTENDANTS.EXPIRY_DAYS) }
          }
        ]
      })
    }
    ...
    if (reqData.facilityId) {
      nurseMatch.$match.facilityId = { $in: [new mongoose.Types.ObjectId(reqData.facilityId)] }
    }

    if (reqData.facilityName) {
      match.$match.$and.push({ 'facilities.name': new RegExp(reqData.facilityName, 'i') })
    }
  ...
  static districtsList (req, skipPageLimit = false) {
    ...
    if (reqData.provinceName) {
      match.$match.$and.push({ 'provinces.name': new RegExp(reqData.provinceName, 'i') })
    }

    if (reqData.provinceCode) {
      match.$match.$and.push({ 'provinces.code': new RegExp(reqData.provinceCode, 'i') })
    }
  ...
  static sectorsList (req, skipPageLimit = false) {
    ...
    if (reqData.districtName) {
      match.$match.$and.push({ 'districts.name': new RegExp(reqData.districtName, 'i') })
    }

    if (reqData.districtCode) {
      match.$match.$and.push({ 'districts.code': new RegExp(reqData.districtCode, 'i') })
    }
  ...
  static cellsList (req, skipPageLimit = false) {
    ...
    if (reqData.sectorName) {
      match.$match.$and.push({ 'sectors.name': new RegExp(reqData.sectorName, 'i') })
    }

    if (reqData.sectorCode) {
      match.$match.$and.push({ 'sectors.code': new RegExp(reqData.sectorCode, 'i') })
    }
  ...
  static villagesList (req, skipPageLimit = false) {
    ...
    if (reqData.cellName) {
      match.$match.$and.push({ 'cells.name': new RegExp(reqData.cellName, 'i') })
    }

    if (reqData.cellCode) {
      match.$match.$and.push({ 'cells.code': new RegExp(reqData.cellCode, 'i') })
    }
  ...
  static professionsList (req, query, skipPageLimit = false) {
    ...
    if (reqData.kinyarwandaName) {
      query.where({ kinyarwandaName: new RegExp(reqData.kinyarwandaName, 'i') })
    }
  ...
  static disabilitySpecificList (req, skipPageLimit = false) {
    ...
    if (reqData.disabilityAreaId) {
      specificMatch.$match.disabilityAreaId = { $eq: new mongoose.Types.ObjectId(reqData.disabilityAreaId) }
    }
